...
const cleanupExpired = async () => {
    ...
    } catch (error) { console.error("Błąd czyszczenia:", error); }
};
...
const createProductFields = (data = {}) => {
    ...
    div.innerHTML = `
        <div class="input-group"><label>Nazwa (np. Książka, Jajka, Konsultacja)</label><input type="text" class="p-name" value="${data.name || ''}" placeholder="np. Jajka, Sparing" required></div>
    ...
    // TO PRZYWRACA WIDOCZNOŚĆ ZDJĘĆ PODCZAS EDYCJI
    if (data.imageUrl) {
        div.dataset.oldUrl = data.imageUrl;
    }
    ...
    allListingsData.forEach(item => {
        ...
        card.innerHTML = `
            ...
            ${(d.items || []).map(it => {
                ...
                // ORYGINALNE, DZIAŁAJĄCE WYŚWIETLANIE ZDJĘĆ
...
document.getElementById('listing-form').onsubmit = async (e) => {
    ...
    try {
        if(isEditingOffer) {
            ...
            data.createdAt = new Date(); // Zachowujemy jako obiekt Date (Timestamp), aby stary kod się nie psuł!
...
// ==========================================
// ZARZĄDZANIE OFERTĄ PRZEZ SPRZEDAWCĘ
// ==========================================
...
document.getElementById('btn-edit-offer').onclick = () => {
    ...
    (d.items || []).forEach(it => {
        ...
        // PRZYWRÓCONE PRZEKAZYWANIE ZDJĘCIA PRZY EDYCJI
...
// ==========================================
// RENDEROWANIE I POBIERANIE FACHOWCÓW
// ==========================================
...
onSnapshot(collection(db, "specialists"), (snap) => {
    let specs = [];
    snap.forEach(docSnap => {
        specs.push({ id: docSnap.id, data: docSnap.data() });
    });
    
    // JS SORTOWANIE DLA FACHOWCÓW
    specs.sort((a, b) => {
        const dateA = a.data.createdAt?.toDate ? a.data.createdAt.toDate() : new Date(a.data.createdAt || 0);
        const dateB = b.data.createdAt?.toDate ? b.data.createdAt.toDate() : new Date(b.data.createdAt || 0);
        return dateB - dateA;
    ...
    specs.forEach(item => {
        ...
        cont.innerHTML += `
            <div class="product-card" style="padding:0;">
                <button onclick="window.deleteSpecialist('${item.id}', '${d.pin}')" style="position:absolute; top:15px; right:15px; border:none; background:#fef3c7; color:#d97706; font-weight:bold; border-radius:8px; padding:5px 10px; cursor:pointer; font-size:0.7rem; z-index:10;">Usuń</button>
